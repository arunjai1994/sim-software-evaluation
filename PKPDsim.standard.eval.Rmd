...
#### N/A - AFAIK author has not implimented meaningful covariate modeling
What we could do is specify the covariate as a parameter, and then attatch a CV to it using omega (basically treat it like CL, V, KA, K12, etc).  I think this is against the spirit of covariates and will not pursue this route unless asked

Covariate models must all be specified in CPP code! - complication

**cpp does NOT have an exponential operator.** This makes it very difficult to impliment allometry!!!  I do not think we can load libraries inside of the "code" arg: we need to use {c++} pow() which is not standard. 

```{r}


##CrCl   <- new_covariate(value = 86.1)


                                    
##pk4 <- new_ode_model(code = '             
#                        dAdt[1] = -(0.67 * CRCL)/(V) * A[1] + rate
#                    ',
#                     covariates = list("CRCL" = CrCl),
#                     cpp_show_code = TRUE)

##dat6 <- sim_ode (ode = "pk4",
#                par = p,
#                n_ind = 50,
#                omega = cv_to_omega(par_cv = list(V = .7, CrCl =           .6),      parameters = list(CL = 'CL', V = 'V', CrCl =                           'CRCL')),
#                covariates = list('CRCL' = CrCl),
#                regimen = r1 )


##ggplot(dat6, aes(x=t, y=y, group=id, colour=factor(id))) +
#  scale_colour_discrete(guide=FALSE) +
#  geom_line() +
#  facet_wrap(~comp, scales = 'free') +
#  theme_empty()
...
-We could enhance the shiny app to allow model parameters to be inputted and selected - much improved over phoenix, and useful for PK students!
-I could probably replicate this library in python - would need lots of help.  Cython >> rcpp IMO
