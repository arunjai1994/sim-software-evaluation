...
* **Lets make a standard PopPK test subject number (i say 1000) and time the simulators!!!!**
...
#### Loop
*RxODE is the only thing that requires looping outside of compiled code.
*lighting fast.  5 mississippi for 10k subjects (i'll test exactly when I have time lol )
...
##Covariate modeling
Nothing special.  Does not have an specific PKPDsim class for covariate (though it PKPDsim has questionable functionality), nor does it have a function like `mrgsolve::idata()' to pipe updated params.  
#####Options 
